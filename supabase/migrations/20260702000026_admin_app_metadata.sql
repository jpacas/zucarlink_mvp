-- Fix de seguridad: el rol admin se decidía con user_metadata.is_admin, que el propio
-- usuario puede auto-asignarse vía supabase.auth.updateUser({ data: { is_admin: true } }).
-- Se migra la fuente de verdad a app_metadata, que solo puede escribir el service_role.

-- 1. Redefinir public.is_admin() para leer app_metadata en vez de user_metadata.
...
-- 2. Recrear funciones que tenían el chequeo de admin inline con user_metadata,
-- para que usen public.is_admin() (app_metadata) en su lugar.
...
-- 3. Backfill one-shot: copiar is_admin=true de user_metadata a app_metadata,
-- y limpiar el claim inseguro de user_metadata.
update auth.users
set raw_app_meta_data = coalesce(raw_app_meta_data, '{}'::jsonb) || jsonb_build_object('is_admin', (raw_user_meta_data->>'is_admin')::boolean)
where (raw_user_meta_data->>'is_admin')::boolean is true;
